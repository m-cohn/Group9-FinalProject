...
class MediumweightCNN_padded(nn.Module):
    def __init__(self, use_maxpool=True):
        super(MediumweightCNN_padded, self).__init__()
        self.use_maxpool = use_maxpool
...
class MediumweightCNN_MFCC(nn.Module):
    def __init__(self, use_maxpool=True):
        super(MediumweightCNN_MFCC, self).__init__()
        self.use_maxpool = use_maxpool
        ...
        num_nodes = 2560 if self.use_maxpool else 56960
        self.fc1 = nn.Linear(num_nodes, 256)
        self.dropout = nn.Dropout(p=0.5)
        self.fc2 = nn.Linear(256, 1)
